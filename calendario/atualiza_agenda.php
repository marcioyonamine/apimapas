...
for($i = 0; $i < count($x); $i++){
	...
	if($y == FALSE){
		$sql = "INSERT INTO `mapas` (`id`, `id_mapas`, `tipo`, `valor`) VALUES (NULL, '".$x[$i]['id']."', '2', '".addslashes($x[$i]['name'])."')"
;
		$query = mysqli_query($con,$sql);
		if($query){
...
for($i = 0; $i < count($ccsp); $i++){ // roda todos os eventos (1)
	...
	for($k = 0; $k < count($e); $k++){ //roda as ocorrências (2)
		...
		case 'weekly':
			...
			for($t = 0; $t < 6; $t++){
				if(isset($e[$k]['rule']['day'][$t])){				
					array_push($sem_array,$t);
				}
			}
			var_dump($sem_array)."<br />";
			
			while($dia != $fim){
				$semana = diasemananumero($dia);
				if($ccsp[$i]['updateTimestamp'] == ""){
					$date_update = $ccsp[$i]['createTimestamp']['date'];	
				}else{
					$date_update = $ccsp[$i]['updateTimestamp']['date'];	
